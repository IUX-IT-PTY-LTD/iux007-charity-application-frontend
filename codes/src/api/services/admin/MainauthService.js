 ...
 */
export const setAuthToken = (token, expiryDays = 7) => {
  if (typeof document === 'undefined') return;

  const expiryDate = new Date();
  expiryDate.setDate(expiryDate.getDate() + expiryDays);
  document.cookie = `${AUTH_TOKEN_KEY}=${token}; expires=${expiryDate.toUTCString()}; path=/`;
};
...
export const register = async (userData) => {
  try {
    ...
    // Optionally set token if returned with registration
    if (data.status === 'success' && data.data && data.data.access_token) {
      setAuthToken(data.data.access_token);
    }
